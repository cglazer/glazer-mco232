		...
		booklist[4] = new String[]{"Frog and Toad"};
		 ...
		 //2. invoke a method to display each student's name , alongside of which display the books that the student read
		   ...
		   displayNamesAndBooksRead(students, qtyStudents, booklist);
		     ...
		     System.out.println("which student's reading list would you like to review?");
			...
			//names[i]= nameCounter++;
		     ...
		     for (int i=0; i<books.length; i++){
			  for(int j=0; j<books[i].length; j++)
		    	 // books[0][i]=names[i];
		        System.out.println(names[i] + " " + books[i][j]);
			     
			  sorry..Mind blanked totally blanked out and didnt have enough time to properly work through and finish:(
		     }
	}
